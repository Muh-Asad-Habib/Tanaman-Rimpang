...
export default async function Detail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!plant) notFound();
  return <main id="konten" className="container page-main"><Link href="/jelajah" className="back-link"><Icon name="arrow" style={{ transform: "rotate(180deg)" }} size={17} /> Kembali ke koleksi</Link>
    <div className="detail-grid"><div className="detail-art" style={{ background: plant.color }}><span className="specimen-number">SPESIMEN {String(plant.id + 1).padStart(2, "0")}</span><Image src={plant.image} alt={`Contoh rimpang ${plant.name}`} fill sizes="(max-width: 800px) 90vw, 50vw" /><span className="detail-caption">Setiap rimpang punya karakter.</span></div>
